...
@SupportedAnnotationTypes("com.yxy.apt_annotation.ViewCreator")
...
public class MyViewCreatorProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        if(mViewNameSet == null || mViewNameSet.isEmpty()){
            System.out.println("mViewNameSet == null");
            return false;
        }
    ...
    private void startGenerateCode(Set<String> mViewNameSet) {
        ...
        TypeSpec myViewCreatorImpl = TypeSpec.classBuilder("MyViewCreatorImpl")
                ...
                .addModifiers(Modifier.PUBLIC)
                //实现接口
                .addSuperinterface(ClassName.get("com.yxy.apt_api", "IMyViewCreator"))
                //添加方法
        ...
        //=========================================== 指定包路径，构建文件体 start =================================、
        //指定类包路径
        JavaFile javaFile = JavaFile.builder("com.yxy.viewopt",myViewCreatorImpl).build();
        //生成文件
        try {
    ...
    private Set<String> readViewNameFromFile() {
        try {
            //获取存储控件名称的文件
            File file = new File("/Users/zyb/Downloads/AptDemo-master/all_view_name.txt");
